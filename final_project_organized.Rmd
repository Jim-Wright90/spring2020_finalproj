...
batch <- batch %>% 
  mutate(fr_urban = as.factor(fr_urban), # MH comment: 'fr_ubran' seems like it might be a typo but doesnaffect later code
...
numeric_plot <- function(df, x, y){
  ...
  if(!is.numeric({{x}}) & !is.numeric({{y}})){ #MH comment: I think y'all mean to use 'is.numeric()' since 'as.numeric()' throws a warning 
    stop()
  }
  else{  #MH comment: Maybe, to simplify the code, you could use just 'else{}' or 'return()'? I'm not sure if both are needed if they produce the same output but I'm not sure :P
    plot_num
  }
  return(plot_num)
}
...
size_density <- four_year %>% 
  ...
  theme(panel.grid.major.y = element_blank(),
        ...
        panel.grid.major.x = element_line(color = "gray80")) +
  labs(x = "Percentage of Students", 
       y = "School enrollment",  #MH comment: it might be helpful to add a label to the y-axis  
       title = glue("Distribution of School Enrollment\n by Students Scoring Below 15th Percentile on Standardized Tests:\n {.x}")) +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text=element_text(size=10),
        axis.title=element_text(size=10))))
