...
### Nest and Mutate (Claire)

I'll use the original four years' datasets (05-06, 07-08, 15-16, 17-18) downloaded directly from the NCES website, https://nces.ed.gov/surveys/ssocs/, and select variables of interest that are available across the four years. These variables include urbanicity, school size, percentage of low-performing students, percentage of college-going students, and total numbers of a) incidents reported, b) incidents reported to police, and c) out-of-school suspensions.

step 1, prepare four-year data:

```{r}
sf06 <- import(here("spss_files", "pu_ssocs06_spss.sav"),
               setclass = "tbl_df") %>%
        select(urbanicity = FR_LOC4,
               size = FR_SIZE,
               low_performing = C0532,
               ...
               suspension = OUTSUS06) %>% 
        mutate(year = "05-06")
sf08 <- import(here("spss_files", "pu_ssocs08_spss.sav"),
               setclass = "tbl_df") %>% 
        select(urbanicity = FR_URBAN,
               size = FR_SIZE,
               low_performing = C0532,
               ...
               suspension = OUTSUS08)%>% 
        mutate(year = "07-08")
sf16 <- import(here("spss_files", "pu_ssocs16.sas7bdat"),
               setclass = "tbl_df")%>% 
        select(urbanicity = FR_URBAN,
               size = FR_SIZE,
               low_performing = C0532,
               ...
               suspension = OUTSUS16)%>% 
        mutate(year = "15-16")
sf18 <- import(here("spss_files", "pu_ssocs18.sav"),
               setclass = "tbl_df")%>% 
        select(urbanicity = FR_URBAN,
               size = FR_SIZE,
               low_performing = C0532,
...
four_year <- bind_rows(sf06, sf08, sf16, sf18)%>%
             pivot_longer(cols = c(incidents, incidents_police, suspension),
                          names_to = "safty_indicators",
                          values_to = "total")
```

step 2, nest by year and visualize school safety, size, and urbanicity

```{r}
plot_1 <- function(x){
  x %>% 
    ggplot(aes(size, total))+
    geom_col(aes(fill = safty_indicators),
             position = "dodge")+
    facet_wrap(~urbanicity)+
    theme_minimal()+
    scale_fill_brewer(palette = "Set2")+
...
step 3, visualize school safety, low-performing students, and college-going students
...
walk2(paths, by_year$plot, ggsave,
      width = 9.5, 
      height = 6.5,
      dpi = 500)
```
