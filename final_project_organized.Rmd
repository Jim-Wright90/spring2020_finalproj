...
```{r iteration  - another way to make plot for each year}
tmpt_data <- four_year %>% 
  group_by(year) %>% 
  #nest() %>% 
  mutate(label = 
...
final_plots <- tmpt_data %>% 
  group_by(year, label) %>% 
  nest() %>% 
  mutate(plots = pmap(list(year, label, data),
                      ~ggplot(..3, aes(size, total))+
                        geom_col(aes(fill = safety_indicators),
                         position = "dodge")+
                facet_wrap(~urbanicity)+
                theme_minimal()+
                scale_fill_brewer(palette = "Set2")+
                labs(title = glue("School Safety, School Size, and Urbanicity in year {..1}"),
                     x = "School Size",
                     y = "Total Number of Cases",
                     fill = "Safety Indicators",
                     caption = glue("Survey result for {..2} only")))
)
