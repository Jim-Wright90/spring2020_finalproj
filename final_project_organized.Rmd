...
college_bound_hist <- four_year %>% 
  ...
  labs(x = "Percentage of Students College Bound",
       y = "Total",
       title = glue("Distribution of School Urbanicity by College Bound Students: {.x}")) +
...
size_hist <- four_year %>% 
  ...
  labs(x = "Percentage of Students College Bound",
       y = "Total",
       title = glue("Distribution of School Enrollment by College Bound Students: {.x}")) +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text = element_text(size = 10),
        axis.title=element_text(size=10))))
...
comp <- anova(year_ind$mod1[[1]], year_ind$mod2[[1]])
comp
...
year_ind %>%
    pivot_longer(mod1:mod2, names_to = "model", values_to = "output")

year_ind %>%
    gather(model, output, mod1:mod2) %>%
    mutate(r2 = map_dbl(output, ~summary(.x)$r.squared))
